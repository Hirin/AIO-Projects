#!/usr/bin/env python3
"""
Generate unified-ablation.ipynb with toggle system
"""

import json

def create_notebook():
    """Create the unified ablation notebook with all toggles."""
    
    cells = []
    
    # Cell 1: Title and Preset Configs
    cells.append({
        "cell_type": "markdown",
        "metadata": {},
        "source": [
    ...
    # Cell 6: Mixup Collate
    cells.append({
        "cell_type": "markdown",
        "metadata": {},
        "source": ["## 3. Augmentation Utilities"]
    })
    ...
    cells.append({
        "cell_type": "markdown",
        "metadata": {},
        "source": ["## 5. WARMUP Mode (Test All Pipelines)"]
    })
    ...
    cells.append({
        ...
        "source": [
            ...
            "        else:\n",
            "            # No test labels - show val accuracy if available\n",
            "            if val_ds:\n",
            "                val_acc = evaluate_val(model, val_loader)\n",
            "                status = '>>> BEST' if val_acc > best_acc else ''\n",
            "                if val_acc > best_acc:\n",
            "                    best_acc = val_acc\n",
            "                    torch.save(model.state_dict(), 'best_p1.pt')\n",
            "                print(f'Ep {epoch}/{EPOCHS_P1}: L={loss:.4f} TrAcc={train_acc:.4f} ValAcc={val_acc:.4f} {status}')\n",
            "            else:\n",
            "                torch.save(model.state_dict(), 'best_p1.pt')\n",
            "                print(f'Ep {epoch}/{EPOCHS_P1}: L={loss:.4f} TrAcc={train_acc:.4f}')\n",
            "        \n",
            "        gc.collect()\n",
            "        torch.cuda.empty_cache()"
        ]
    ...
    cells.append({
        ...
        "source": [
            ...
            "                    best_acc = val_acc\n",
            "                    torch.save(model.state_dict(), 'best_final.pt')\n",
            "                print(f'P2 Ep {epoch}/{EPOCHS_P2}: L={loss:.4f} TrAcc={train_acc:.4f} ValAcc={val_acc:.4f} {status}')\n",
            "            else:\n",
            "                torch.save(model.state_dict(), 'best_final.pt')\n",
            "                print(f'P2 Ep {epoch}/{EPOCHS_P2}: L={loss:.4f} TrAcc={train_acc:.4f}')\n",
            "        \n",
            "        gc.collect()\n",
            "        torch.cuda.empty_cache()\n",
            "    \n",
    ...
    cells.append({
        "cell_type": "markdown",
        "metadata": {},
        "source": ["## 7. Visualization & Analysis"]
    })
...
if __name__ == "__main__":
    ...
    output_path = "/mnt/hdd/Learning/AIO-Projects/Project-7.1/unified-ablation.ipynb"
